...
\end{equation}
using the Cholesky factorization algorithm and then compares the result with the one found using the QR factorization of the matrix $A$. To begin with, we report the functions defined in the library \mintinline{python}{Project_2.py}.

\begin{minted}[mathescape, linenos, breaklines]{python}
def compute_A(points,order):
  r'''This function computes the Vandermonde matrix A taking as inputs an array, containing the coordinates of the points, and an integer representing the order of the polynomial.
...
def LeastSquare_Cholesky(A, b):
  r''' This function computes the Cholesky factorization of a matrix :math:`(A^{T}A)`, and solves the linear system :math:`(A^{T}Ax = A^{T}b)`, giving as output the solution of the system.
...
def LeastSquare_QR(A, b):
  r''' This function computes the QR factorization of a matrix :math:`A`, and solves the linear system :math:`Rx = Q^{T}b`, giving as output the solution of the system.
...
%we report the main program where, using the functions defined before, we computed the solution of the minimization problem
%of the linear system $A\textbf{x} = \textbf{b}$ using both the Cholesky and the QR factorization.
\begin{minted}[mathescape, linenos, breaklines]{python}
...
%Using the previous functions we have computed the solution to the minimization problem, obtaining the following results:\\
...
%\noindent Knowing that $\text{Null}(A^T) = (\text{Ran}(A))^\perp$ we can, immediately, find the solution. 
In fact:
\begin{equation}\label{key}
...
%
%\begin{equation}\label{key}
%	\tilde{r}_{11} = -\text{sgn}(a_{11})\lVert \textbf{a}_1\rVert_2 =  - \lVert \textbf{a}_1\rVert_2 = -1.85
%\end{equation}
Then:
\begin{equation}
	\tilde{\hat{\textbf{u}}}_1 = 
...
%
%\begin{equation}\label{key}
%	\tilde{\hat{\textbf{u}}}_1  = \textbf{a}_1 - r_{11} \textbf{e}_1 = \begin{pmatrix}
%		1.07\\
%		1.07\\
%		1.07
...
\end{equation}
%\begin{equation}\label{key}
%	\lVert \hat{\textbf{u}_{1}} \rVert_2 = \sqrt{8.53 + 1.14 + 1.14} = \sqrt{10.81} = 3.29
%\end{equation}
%\begin{equation}\label{key}
%		\textbf{u}_1 = 	\hat{\textbf{u}}_1/\lVert \hat{\textbf{u}_{1}} \rVert_2  =  \begin{pmatrix}
%			0.888\\
%			0.325\\
%			0.325
%		\end{pmatrix} 
...
\begin{equation}\label{key}
	...
	\end{pmatrix}.
\end{equation}
%We can observe that the matrix $\tilde{R}$ is full rank, therefore it is possible to use the QR factorization to solve the least square problem. It is important to stress that, as $A^T A$ is indefinite in $3$ digit precision, using Cholesky is not possible.\\
...
%
%
%It is important to point out that the columns of $\tilde{Q}$ are not exactly orthonormal.\\
...
%\end{equation}
%where $ \textbf{x}^*$ is the solution to the least square problem, 
and compute:
\begin{equation}\label{key}
...
\begin{figure}[H]
	\centering
	\subfloat[][n=10]{\includegraphics[scale=0.21]{Plot/Singular_values_of_R_for_n=10}} \qquad \subfloat[][n=20]{\includegraphics[scale=0.21]{Plot/Singular_values_of_R_for_n=20}} \\
	\subfloat[][n=50]{\includegraphics[scale=0.21]{Plot/Singular_values_of_R_for_n=50}} \qquad
	\subfloat[][n=100]{\includegraphics[scale=0.21]{Plot/Singular_values_of_R_for_n=100}}
