...
if False:
    ...
    x0 = np.array([0.1,0.1])
    method = 'basic'
    results = int_point(func_b, grad_b, hess_b, c_b, grad_c_b, hess_c_b, x0, method=method, 
                        alpha=1., beta=1., gamma=1., mu=mu, tol=tol, maxit=100, curv = True, seed=seed)
...
x0_list = [np.array([0.1,0.1]),np.array([0.5,0.5])]
mu_list = [1e-12]
colorplot = True
###### COLORPLOT #######
if colorplot == True:
        ...
        for x0 in x0_list:
            ...
            for l_int in l_array:
                for z_int in z_array:
                    ...
                    # if ((abs(min_val)) < 1e-12).all():
                    # if np.linalg.norm(min_val-min_exact) > tol_x and np.linalg.norm(min_val-min_err) < tol_x:
                    if np.linalg.norm(min_val-min_err) < tol_x:
                        k = 150
                        # print(f'x0={x0}, l0={l0}, z0={z0}, min_val={min_val}')
            ...
            plt.title(fr'$\mu$ = {mu}, $x_{{0}}$ = {x0}, curv = {curv}')
            plt.xlabel(r'$\lambda$')
            plt.ylabel("z")
            plt.colorbar(label = 'k')
            if curv == True:
                fig.savefig(f'{common_path}_latex\\Plot\\func_b\\colorplot\\func_b_with_K_method={method}_x0={x0}_mu={mu}_tol={tol_x}.png', bbox_inches='tight', dpi = 500)
            else:
                fig.savefig(f'{common_path}_latex\\Plot\\func_b\\colorplot\\func_b_method={method}_x0={x0}_mu={mu}_tol={tol_x}.png', bbox_inches='tight', dpi = 500)

x0_list = [np.array([0.1,0.9])]
l_0 = np.array([0.6, 0.6, 0.6])
z_0 = np.array([9.3, 9.3, 9.3])
###### CONTOURPLOT ######
if False:
    for x0 in x0_list:
        for mu in mu_list:
            results = int_point(func_b, grad_b, hess_b, c_b, grad_c_b, hess_c_b, x0, method=method, alpha=1., beta=1.,
                            gamma=1., mu=mu, l0=l_0, z0=z_0, tol=tol, maxit=100, curv=True, seed=seed)
            k = results['n_iter']
            conv = results['convergence']
